      ...
      <div class="col-md-8">
          <div class="my-3 p-3 bg-body rounded shadow-sm" style="width:100%;">
              <div style="display: flex; align-items: center;">
                  ...
                  <h3 style="margin-left: 5px;">édition une ordonnance de traitement</h3>
              </div>
              <div class="mt-4">
                  
                  <form style="width:80%;" method="post"  action="/api/ordonnance/{{$ordonnance->id}}">
                      @csrf
                      @method('PUT')
                          ...
                          <select class="form-select" name="consultation_id">
                            ...
                            @foreach (App\Models\Consultation::all() as $consultation)
                              @if (!in_array($consultation->rdv->patient->id, $selectedPatients))
                                ...
                                @endphp
                                @if ($consultation->id == $ordonnance->consultation_id)
                    ...
                    <div style="display: flex; flex-wrap: wrap; margin-left:80px;">
                        <div class="mb-3" style="flex: 1; margin-right: 10px;">
                            ...
                            <input type="text" class="form-control" name="medicament" value="{{$ordonnance->medicament}}" >
                        </div>
                        <div class="mb-3" style="flex: 1; margin-left: 10px;">
                            <label for="exampleInputPassword1" class="form-label">Dosage:</label>
                            <input type="text" class="form-control" name="dosage" value="{{$ordonnance->dosage}}" >
                        </div>
                    </div>
                    <div style="display: flex; flex-wrap: wrap; margin-left:80px;">
                        <div class="mb-3" style="flex: 1; margin-right: 10px;">
                            <label for="exampleInputPassword1" class="form-label">Nbr de fois par jour:</label>
                            <input type="text" class="form-control" name="nbr_fois" value="{{ $ordonnance->nbr_fois }}" >
                        </div>
                        <div class="mb-3" style="flex: 1; margin-left:10px;">
                            <label for="exampleInputPassword1" class="form-label">Quantité:</label>
                            <input type="text" class="form-control" name="qte" value="{{ $ordonnance->qte }}" >
                        </div>
                    </div>
